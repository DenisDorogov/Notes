...
git remote add origin https://github.com/DenisDorogov/Induclimber.git # добавить удалённый репозиторий (с сокр. именем origin) с указанным URL
...
git commit -am ”comment” # Закоммитить изменения предварительно выполнив: git add .
git checkout -- filename.txt #Отменить изменения файла до последнего состояния.
git push # Залить изменения на удаленный сервер
...
Урок Git. Большой практический выпуск Артем Матяшов 
...
#.gitignore 
#Файл .gitignore сордержин завнания игнорируемых областей папки не попадающих под контроль версий.
...
```
git config # Подскажет какие параметры можно использовать.
git config --list # Выводит все текущие настройки конфига 
...
git push --set-upstream origin master #Команда свяжет локальную ветку master  с удалённой origin/master для упрощённой отправки изменений git push
...
Лучше располагать в корень приложения.
# Игнор-лист файлов проекта
# Игнорировать ВСЕ файлы и директории, включая поддиректории и файлы в них
*
# ---- ФАЙЛЫ ----
...
-----------------< |Как сделать pull request в чужой репозиторий>------------------------
#pull request
